...
import { ArrowUpDown, FileText, MoreHorizontal, User2 } from "lucide-react";
...
import { getInvoicePdfUrl } from "@/lib/actions";
      ...
      cell: ({ row }) => {
        ...
        const handleViewPdf = async () => {
          ...
          try {
            ...
            toast(err instanceof Error ? err.message : "Failed to open PDF");
          }
        };
        ...
        return (
          <Button
            ...
            className="h-8 w-8"
            disabled={!pdfPath}
            title={pdfPath ? "View original PDF" : "No PDF on file"}
            onClick={handleViewPdf}
          >
            <FileText className={`h-4 w-4 ${pdfPath ? "" : "opacity-30"}`} />
          </Button>
        );
      },
    },
